...
//------------------------------------------------------------------------------
bool Fat16::fatGet(fat_t cluster, fat_t* value) {
...
//------------------------------------------------------------------------------
bool Fat16::fatPut(fat_t cluster, fat_t value) {
 ...
 **/
int16_t Fat16::fgets(char* str, int16_t num, char* delim) {
  char ch;
  int16_t n = 0;
  int16_t r = -1;
...
bool Fat16::freeChain(fat_t cluster) {
  while (1) {
    fat_t next;
...
bool Fat16::init(SdCard* dev, uint8_t part) {
  ...
  fat_boot_t* bpb = &cacheBuffer_.fbs;
 ...
 */
bool Fat16::open(const char* fileName, uint8_t oflag) {
 ...
 */
bool Fat16::open(uint16_t index, uint8_t oflag) {
  if (!volumeInitialized_ || isOpen()) return false;
 ...
 * entry.
 */
bool Fat16::readDir(dir_t* dir, uint16_t* index, uint8_t skip) {
  dir_t* p;
 ...
 * or an I/O error occurred.
 */
bool Fat16::remove(const char* fileName) {
  Fat16 file;
 ...
 * the value zero, false, is returned for failure.
 */
bool Fat16::seekSet(uint32_t pos) {
 ...
 * the value zero, false, is returned for failure.
 */
bool Fat16::timestamp(uint8_t flags, uint16_t year, uint8_t month,
 ...
 */
bool Fat16::truncate(uint32_t length) {
  // error if file is not open for write
  if (!(flags_ & O_WRITE)) return false;
